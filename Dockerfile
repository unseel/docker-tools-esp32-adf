...
RUN apt-get update -y && apt-get install -y git wget flex bison gperf python3 python3-pip python3-setuptools cmake ninja-build ccache libffi-dev libssl-dev dfu-util vim && rm -rf /var/lib/apt/lists/*
...
# adf init
RUN cd /root && git clone --recursive https://github.com/espressif/esp-adf.git esp-adf
